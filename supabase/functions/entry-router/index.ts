...
function pilotEnabled(id: string) {
  const pilots = (Deno.env.get("ENTRY_ROUTER_PILOT_IDS") ?? "")
...
Deno.serve(async (request) => {
  ...
  if (!id || !message || !pilotEnabled(id)) {
    return json({ handled: false, pilot: false });
  }
  ...
  const [customer, provider, preference, session] = await Promise.all([
    supabase.from("customers").select(
      "id,registration_status,terms_accepted_at",
    ).eq("phone", id).maybeSingle(),
    supabase.from("handymen").select(
      "id,status,registration_status,terms_accepted_at,verification_status",
    ).eq("phone", id).maybeSingle(),
  ...
  for (const result of [customer, provider, preference, session]) {
    if (result.error) throw result.error;
  }

  const customerState = customer.data?.registration_status === "active" &&
      customer.data?.terms_accepted_at
    ? "active"
    : customer.data
    ? "onboarding"
    : "none";
  const providerState = provider.data?.verification_status === "verified"
    ? "verified"
    : provider.data?.registration_status === "active" &&
        provider.data?.terms_accepted_at
    ? "active"
    : provider.data
    ? "onboarding"
    : "none";
  ...
  if (decision.kind === "restricted") {
    return json({
      handled: true,
      reply:
        "This account is restricted. Contact HandyConnect support for review.",
    });
  }
  if (decision.kind === "guest_home") {
    return json({
      handled: true,
      reply:
        "Welcome to HandyConnect 👋\nExplore services freely. You only confirm your details when you submit a request.",
      ui: {
        type: "list",
  ...
  if (message === "NAV:SERVICES") {
    const skills = await supabase.from("skills").select("name").eq("active", true)
      .order("name").limit(20);
    if (skills.error) throw skills.error;
    const names: string[] = (skills.data ?? []).map((item: { name: string }) =>
      item.name
    );
    return json({
      handled: true,
      reply: names.length
        ? `HandyConnect currently supports:\n\n${names.map((name) => `• ${name}`).join("\n")}\n\nYou can explore without registering. We’ll ask for your name and consent only when you submit a request.`
        : "Service browsing is temporarily unavailable. You can still describe the home repair you need.",
      ui: {
        type: "buttons",
        body: "Ready when you are",
        buttons: [
          { id: "CUSTOMER:REQUEST", title: "Request handyman" },
          { id: "NAV:HELP", title: "How it works" },
  ...
  if (decision.kind === "help") {
    return json({
      handled: true,
      reply:
        "HandyConnect helps customers find suitable, verified local handymen. Providers can apply, verify their identity and receive relevant work.",
      ui: {
        type: "buttons",
        body: "What would you like to do?",
        buttons: [{ id: "ROLE:CUSTOMER", title: "Find a handyman" }, {
          id: "ROLE:PROVIDER",
  ...
  if (decision.kind === "acknowledgement") {
    return json({ handled: true, reply: "👍" });
  }

  if (
    decision.kind === "customer_request" && message !== "REQUEST_HELP" &&
    message !== "CUSTOMER:REQUEST" && message !== "ROLE:CUSTOMER"
  ) {
    ...
    if (started.status >= 300) {
      return json({ ...started.body, handled: true }, started.status);
    }
    const continued = await call(url, key, "job-intake-router", {
      ...input,
      message,
    });
    return json({
      ...continued.body,
      handled: true,
      entry_decision: decision.kind,
    }, continued.status);
  }

  const roleMessage = decision.kind === "customer_home"
    ? "HOME"
    : decision.kind === "provider_home"
    ? "HANDYMAN_HOME"
  ...
  const target = decision.kind === "customer_home"
    ...
    ? "role-router"
    : "marketplace-router";
  const delegated = await call(url, key, target, {
    ...input,
    message: roleMessage,
  });
  return json({
