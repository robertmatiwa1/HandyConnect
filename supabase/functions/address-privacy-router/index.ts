...
function addressUi(rows: any[]) {
  return {
    type: "list",
    body: "Where should the handyman come?",
...
Deno.serve(async (request) => {
  ...
  if (["ji_description", "ji_urgency", "ji_time", "ji_materials", "ji_photo"].includes(state)) {
  ...
  if (state === "ji_location") {
    ...
    if (!location) {
      return json({
        handled: true,
        reply: "Please send at least Suburb, City. Example: Langa, Cape Town",
      });
    }
    ...
    if (customer?.id) {
      ...
      if ((addresses.data ?? []).length) {
        await supabase
          .from("conversation_sessions")
          .update({ state: "ji_saved_address", context })
          .eq("id", session.id);
        return json({
          handled: true,
    ...
    await supabase
      .from("conversation_sessions")
      .update({ state: "ji_address", context })
      .eq("id", session.id);
    return json({
      handled: true,
      reply: "What is the street address?\nIt stays private until a handyman accepts.",
    });
  }

  if (state === "ji_saved_address") {
    if (message === "JI_ADDR_NEW") {
      await supabase.from("conversation_sessions").update({ state: "ji_address" }).eq("id", session.id);
      return json({
        handled: true,
        reply: "What is the street address?\nIt stays private until a handyman accepts.",
      });
    ...
    if (message.startsWith("JI_ADDR:") && customer?.id) {
      ...
      await supabase
        ...
        .eq("id", session.id);
      return json({ handled: true, reply: "Address selected.", ui: urgencyUi });
    }

    return json({ handled: true, reply: "Choose a saved address or use another address." });
  }

  if (state === "ji_address") {
    if (message.length < 5) {
      return json({ handled: true, reply: "Please send the street number and street name." });
    }
    await supabase
      .from("conversation_sessions")
      .update({
      ...
      .eq("id", session.id);
    return json({ handled: true, reply: "Address saved privately.", ui: urgencyUi });
  }
  ...
  const customerCommand =
    ["HOME", "MY_JOBS", "CUSTOMER_JOBS", "CUST_MORE", "CUST_PROFILE", "CUST_ADDRESSES", "CUST_HELP"].includes(message) ||
