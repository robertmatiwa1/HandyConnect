...
declare
  ...
  v_phone text;
begin
  select id,processed_at into v_event_id,v_processed_at
  ...
  if p_event_type='charge.success' then
    ...
    if (v_reference like 'HC-PRO-%' or (coalesce(p_configured_plan_code,'')<>'' and v_event_plan=p_configured_plan_code))
      ...
      select x.payment_id,x.valid_until into v_payment_id,v_valid_until
      ...
      if v_phone is not null then
        ...
        values(v_phone,'payment_success',
          ...
          '{}'::jsonb,'payment-success:'||v_payment_id::text)
    ...
    if v_email<>'' then
      ...
      if v_handyman_id is not null then
        ...
        insert into public.notification_outbox(recipient_phone,kind,body,payload,dedupe_key)
        values(v_phone,'payment_failed','Your HandyConnect Pro renewal payment failed. Your current access remains available until the paid period ends; please update your payment method before then.','{}'::jsonb,'payment-failed:'||p_event_id)
        on conflict(dedupe_key) do nothing;
      end if;
    end if;
  end if;
...
revoke all on function public.process_paystack_event(text,text,jsonb,text) from public,anon,authenticated;
grant execute on function public.process_paystack_event(text,text,jsonb,text) to service_role;
