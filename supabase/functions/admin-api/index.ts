 ...
 try{
  ...
  if(action==="summary"){
   ...
   const Q=await Promise.all([
    ...
    s.from("handyman_verification_documents").select("id",{count:"exact",head:true}).eq("status","pending"),
    ...
    s.from("handyman_verification_documents").select("id,handyman_id,document_type,file_name,mime_type,status,submitted_at,review_notes,handymen(full_name,phone,verification_status)").order("submitted_at",{ascending:false}).limit(50),
   ...
   const [jc,se,ac,hc,av,or,pv,jobs,handymen,verifications,reports,payments,notifications]=Q;
   return json({ok:true,stats:{jobs:jc.count??0,searching:se.count??0,active_jobs:ac.count??0,handymen:hc.count??0,available:av.count??0,open_reports:or.count??0,pending_verifications:pv.count??0},jobs:(jobs.data??[]).map((j:any)=>({...j,customer_name:j.customers?.full_name,customer_phone:j.customers?.phone,skill:j.skills?.name})),handymen:handymen.data??[],verifications:(verifications.data??[]).map((v:any)=>({...v,handyman_name:v.handymen?.full_name,handyman_phone:v.handymen?.phone,handyman_verification_status:v.handymen?.verification_status})),reports:(reports.data??[]).map((r:any)=>({...r,job_description:r.jobs?.description})),payments:(payments.data??[]).map((p:any)=>({...p,handyman_name:p.handymen?.full_name,handyman_phone:p.handymen?.phone})),notifications:notifications.data??[]});
  }
  const id=String(input?.id??"");if(!id)return json({error:"id_required"},400);
  if(action==="get_verification_document"){
   const d=await s.from("handyman_verification_documents").select("media_id,mime_type,file_name").eq("id",id).single();if(d.error)throw d.error;
   const token=Deno.env.get("WHATSAPP_ACCESS_TOKEN")??"",version=Deno.env.get("WHATSAPP_GRAPH_VERSION")??"v26.0";if(!token)return json({error:"whatsapp_media_unavailable"},503);
   const meta=await fetch(`https://graph.facebook.com/${version}/${d.data.media_id}`,{headers:{authorization:`Bearer ${token}`}});if(!meta.ok)return json({error:"media_metadata_failed"},502);const m=await meta.json();const file=await fetch(m.url,{headers:{authorization:`Bearer ${token}`}});if(!file.ok)return json({error:"media_download_failed"},502);const bytes=new Uint8Array(await file.arrayBuffer());if(bytes.length>8*1024*1024)return json({error:"document_too_large"},413);return json({ok:true,mime_type:file.headers.get("content-type")||d.data.mime_type||"application/octet-stream",file_name:d.data.file_name||"verification-document",data_base64:b64(bytes)});
  }
  if(action==="approve_verification"||action==="reject_verification"){
   const d=await s.from("handyman_verification_documents").select("id,handyman_id,status").eq("id",id).single();if(d.error)throw d.error;const approved=action==="approve_verification",notes=String(input?.notes??"").slice(0,1000)||null,now=new Date().toISOString();
   const ud=await s.from("handyman_verification_documents").update({status:approved?"approved":"rejected",review_notes:notes,reviewed_at:now}).eq("id",id);if(ud.error)throw ud.error;
   const uh=await s.from("handymen").update(approved?{verification_status:"verified",verified_at:now,verification_notes:notes,updated_at:now}:{verification_status:"rejected",verification_notes:notes,availability_status:"offline",available_until:null,updated_at:now}).eq("id",d.data.handyman_id);if(uh.error)throw uh.error;
   if(approved)await s.from("entitlements").insert({handyman_id:d.data.handyman_id,entitlement_type:"verified_badge",source_type:"admin",source_id:id,status:"active"}).then(()=>{});
